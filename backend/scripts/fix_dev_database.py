...
def main():
    ...
    if test_tenant:
        ...
        with connection.cursor() as cursor:
            cursor.execute("DROP SCHEMA IF EXISTS test_tenant CASCADE")
    ...
    if acme_tenant is None:
        ...
        Domain.objects.create(tenant=acme_tenant, domain="acme.localhost", is_primary=True)
        print("✓ Created acme.localhost → acme (primary)")

        Domain.objects.create(tenant=acme_tenant, domain="acme.django-01.local", is_primary=False)
        print("✓ Created acme.django-01.local → acme")
    else:
        print("✓ acme tenant already exists")
        # Ensure domains exist
        Domain.objects.get_or_create(
            domain="acme.localhost", defaults={"tenant": acme_tenant, "is_primary": True}
        )
        Domain.objects.get_or_create(
            domain="acme.django-01.local", defaults={"tenant": acme_tenant, "is_primary": False}
        )
    ...
    with schema_context("acme"):
        user, created = User.objects.get_or_create(
            username="jwt", defaults={"email": "jwt@example.com", "is_active": True}
        )
